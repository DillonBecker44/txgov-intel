...
import { TechTag, TagDrawer } from '../components/TechTagDrawer';

const NAV_TABS = ['Overview', 'Agency Table', 'SB1 & HB500', 'Budget Tracker', 'Contract AI'];

// Helper — get label string from tag (string or object)
const tagLabel = (t) => typeof t === 'string' ? t : t.label;
...
export default function Dashboard({ onLogout }) {
  ...
  const [sortCol, setSortCol]       = useState('fy26_budget');
  const [sortDir, setSortDir]       = useState(-1);
  const [modal, setModal]           = useState(null);       // legacy AgencyModal fallback
  const [activeTag, setActiveTag]   = useState(null);       // {tag, agency} for TagDrawer
  ...
  const filtered = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      rows = rows.filter(r =>
        ...
        (r.tech_areas || []).some(t => tagLabel(t).toLowerCase().includes(q))
      );
    }
    if (catFilter) rows = rows.filter(r => r.category === catFilter);
    if (activeTech.size) {
      rows = rows.filter(r =>
        (r.tech_areas || []).some(t => activeTech.has(tagLabel(t)))
      );
    }
    rows.sort((a, b) => {
      let av = a[sortCol] ?? -1;
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#F4F6FB', fontFamily:"'DM Sans',-apple-system,sans-serif" }}>
      ...
      {/* ── Header ─────────────────────────────────────────────────────────── */}
      <header style={{ background:'#1E2761', borderBottom:'3px solid #C8A951', position:'sticky', top:0, zIndex:100 }}>
        <div style={{ maxWidth:1400, margin:'0 auto', padding:'0 24px' }}>
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', height:60, gap:16 }}>
            <div style={{ display:'flex', alignItems:'center', gap:12 }}>
              <div style={{ width:36, height:36, background:'#C8A951', borderRadius:6, display:'flex', alignItems:'center', justifyContent:'center', fontSize:20, fontWeight:700, color:'#1E2761', fontFamily:'Georgia,serif', flexShrink:0 }}>S</div>
              <div>
            ...
            <button onClick={onLogout} style={{ padding:'6px 14px', borderRadius:6, border:'1px solid rgba(255,255,255,0.2)', background:'transparent', color:'rgba(255,255,255,0.6)', fontSize:11, cursor:'pointer', fontFamily:'inherit' }}>
              Sign out
            </button>
          </div>
        </div>
      ...
      {tab === 'Overview' && (
        <div style={{ maxWidth:1400, margin:'0 auto', padding:'28px 24px' }}>
          {/* Hero */}
          <div style={{ background:'linear-gradient(135deg,#141A47 0%,#1E2761 60%,#2A3570 100%)', borderRadius:16, padding:'32px', marginBottom:24, border:'1px solid rgba(200,169,81,0.2)' }}>
            ...
            <p style={{ color:'#CADCFC', fontSize:13, maxWidth:660, lineHeight:1.6, marginBottom:24 }}>
              Comprehensive IT budget intelligence, SB1 appropriations, HB500 supplemental funding,
              exceptional items, and strategic initiatives across {SUMMARY_STATS.total_agencies}+ Texas state agencies.
              Data current as of {SUMMARY_STATS.data_updated}.
            </p>
            <div style={{ display:'flex', gap:16, flexWrap:'wrap' }}>
              {[
                ...
                { val:'$135.5M',  lbl:'Texas Cyber Command (New)' },
                { val:`${SUMMARY_STATS.sb1_it_items}`, lbl:'SB1 IT Riders & Provisions' },
                { val:`${SUMMARY_STATS.hb500_it_sections}`, lbl:'HB500 IT Sections' },
              ].map(s => (
                <div key={s.lbl} style={{ background:'rgba(255,255,255,0.07)', border:'1px solid rgba(200,169,81,0.3)', borderRadius:10, padding:'14px 18px', minWidth:130 }}>
          ...
          {/* Top agencies by FY26 budget */}
          <h2 style={{ fontSize:16, fontWeight:600, color:'#1E2761', marginBottom:14 }}>Top Agency IT Budgets — FY2026</h2>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))', gap:12, marginBottom:28 }}>
            {AGENCIES
              .filter(a => a.fy26_budget)
              .sort((a,b) => b.fy26_budget - a.fy26_budget)
              .slice(0,12)
              .map(a => (
                <div key={a.id} style={{ background:'#FFFFFF', borderRadius:12, padding:'16px', border:'1px solid rgba(30,39,97,0.1)', boxShadow:'0 2px 8px rgba(30,39,97,0.06)' }}>
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:10 }}>
                    ...
                    <div style={{ textAlign:'right' }}>
                      <div style={{ fontSize:18, fontWeight:700, color:'#C8A951', fontFamily:'Georgia,serif' }}>{FMT(a.fy26_budget)}</div>
                      <div style={{ fontSize:9, color: a.budget_confirmed ? '#27500A' : '#8A93B2' }}>
                        {a.budget_confirmed ? '✓ Confirmed' : '~ Est.'}
                      </div>
                    </div>
                  </div>
                  <SpendBar value={a.fy26_budget} max={600} />
                </div>
              ))}
          </div>

          <div style={{ background:'#FFFFFF', borderRadius:10, padding:'14px 16px', border:'1px solid rgba(30,39,97,0.1)', fontSize:11, color:'#8A93B2', lineHeight:1.6 }}>
            <strong style={{ color:'#1E2761' }}>Sources:</strong> SB1 89th Legislature GAA (2026-27);
            HB500 Supplemental Appropriations (enacted June 22, 2025); Agency LARs FY2026-27;
            LBB Conference Committee Issue Dockets; DIR Agency Strategic Plan FY2025-29; LBB Fiscal Size-Up 2026-27.
            Est. figures are directional — see budget_source on each agency for methodology.
          </div>
        </div>
      )}
      ...
      {tab === 'Agency Table' && (
        <div style={{ maxWidth:1400, margin:'0 auto', padding:'24px' }}>
          ...
          {/* Controls */}
          <div style={{ background:'#FFFFFF', borderRadius:12, padding:'16px', border:'1px solid rgba(30,39,97,0.1)', marginBottom:16, position:'sticky', top:63, zIndex:80, boxShadow:'0 2px 8px rgba(30,39,97,0.06)' }}>
            <div style={{ display:'flex', gap:10, marginBottom:12, flexWrap:'wrap' }}>
              <div style={{ flex:1, minWidth:220, position:'relative' }}>
                ...
                <input value={search} onChange={e => setSearch(e.target.value)}
                  ...
                  style={{ width:'100%', padding:'8px 12px 8px 32px', fontSize:13, border:'1px solid rgba(30,39,97,0.15)', borderRadius:8, background:'#F4F6FB', color:'#1A1F3C', outline:'none', fontFamily:'inherit', boxSizing:'border-box' }} />
              </div>
              <select value={catFilter} onChange={e => setCatFilter(e.target.value)}
                style={{ padding:'8px 12px', fontSize:13, border:'1px solid rgba(30,39,97,0.15)', borderRadius:8, background:'#F4F6FB', color:'#1A1F3C', outline:'none', cursor:'pointer', fontFamily:'inherit' }}>
            ...
            <div style={{ fontSize:11, fontWeight:500, color:'#8A93B2', textTransform:'uppercase', letterSpacing:0.8, marginBottom:7 }}>
              Filter by technology area — click a tag ↗ for sourced details
            </div>
            <div style={{ display:'flex', flexWrap:'wrap', gap:6 }}>
              {TECH_AREAS.map(t => (
                <button key={t.label} onClick={() => toggleTech(t.label)} style={{
                  ...
                  fontWeight:  activeTech.has(t.label) ? 500 : 400,
                  transition:'all 0.15s',
                }}>{t.label}</button>
              ))}
              {activeTech.size > 0 && (
                <button onClick={() => setActiveTech(new Set())} style={{ padding:'4px 12px', borderRadius:20, border:'1px solid rgba(200,169,81,0.4)', background:'rgba(200,169,81,0.05)', color:'#C8A951', fontSize:11, cursor:'pointer', fontFamily:'inherit' }}>
                  Clear all
                </button>
              )}
            </div>
          ...
          <div style={{ background:'#FFFFFF', borderRadius:12, border:'1px solid rgba(30,39,97,0.1)', overflow:'hidden', boxShadow:'0 2px 8px rgba(30,39,97,0.06)' }}>
            <div style={{ overflowX:'auto' }}>
              <table style={{ width:'100%', borderCollapse:'collapse', tableLayout:'fixed' }}>
                ...
                <thead>
                  <tr>
                    {[
                      ...
                      { key:'fy26_budget',label:'FY26 Budget' },
                      { key:'_init',      label:'Key Initiatives' },
                      { key:'_tech',      label:'Technology Areas — click ↗ tag for sourced details' },
                    ].map(col => (
                      <th key={col.key} onClick={() => handleSort(col.key)} style={{
                        background:'#1E2761',
                        color: sortCol===col.key ? '#C8A951' : 'rgba(255,255,255,0.7)',
                ...
                <tbody>
                  {filtered.length === 0 ? (
                    <tr><td colSpan={4} style={{ textAlign:'center', padding:40, color:'#8A93B2', fontSize:14 }}>
                    ...
                    <tr key={a.id} style={{ borderBottom:'1px solid rgba(30,39,97,0.08)' }}
                      ...
                      <td style={{ padding:'11px 14px', verticalAlign:'top' }}>
                        <div style={{ fontWeight:600, fontSize:13, color:'#1E2761', lineHeight:1.3 }}>{a.name}</div>
                        <div style={{ fontSize:10.5, color:'#8A93B2', marginTop:2 }}>{a.category}</div>
                        <div style={{ marginTop:5, display:'flex', gap:4, flexWrap:'wrap' }}>
                          {(a.sb1_items||[]).length > 0 && (
                            <span style={{ fontSize:9, padding:'2px 6px', borderRadius:4, background:'#E6F1FB', color:'#0C447C', fontWeight:600 }}>SB1 ({a.sb1_items.length})</span>
                          )}
                          {(a.hb500_items||[]).length > 0 && (
                            <span style={{ fontSize:9, padding:'2px 6px', borderRadius:4, background:'#EAF3DE', color:'#27500A', fontWeight:600 }}>HB500 ({a.hb500_items.length})</span>
                          )}
                          {(a.exceptional_items||[]).length > 0 && (
                            <span style={{ fontSize:9, padding:'2px 6px', borderRadius:4, background:'#FAEEDA', color:'#633806', fontWeight:600 }}>{a.exceptional_items.length} EI</span>
                          )}
                        </div>
                      </td>
                      ...
                      <td style={{ padding:'11px 14px', verticalAlign:'top' }}>
                        {a.fy26_budget ? (
                          <>
                            <div style={{ fontSize:13, fontWeight:600, color:'#1E2761', fontFamily:'monospace' }}>{FMT(a.fy26_budget)}</div>
                            <div style={{ fontSize:10, color: a.budget_confirmed ? '#27500A' : '#8A93B2', marginTop:2 }}>
                              {a.budget_confirmed ? '✓ Confirmed' : '~ Est.'}
                            </div>
                            {a.fy27_budget && a.fy27_budget !== a.fy26_budget && (
                              <div style={{ fontSize:10, color:'#8A93B2', marginTop:1 }}>FY27: {FMT(a.fy27_budget)}</div>
                            )}
                          </>
                        ) : <span style={{ fontSize:11, color:'#8A93B2' }}>See notes</span>}
                      </td>
                      ...
                      {/* Tech tags — clickable for sourced drawer */}
                      <td style={{ padding:'11px 14px', verticalAlign:'top' }}>
                        <div style={{ display:'flex', flexWrap:'wrap', gap:4 }}>
                          {(a.tech_areas||[]).map(t => (
                            <TechTag
                              key={tagLabel(t)}
                              tag={t}
                              active={activeTech.has(tagLabel(t))}
                              onClick={(tag) => setActiveTag({ tag, agency: a })}
                            />
                          ))}
                        </div>
                      </td>
          ...
          <p style={{ fontSize:11, color:'#8A93B2', marginTop:10, lineHeight:1.6 }}>
            <strong style={{ color:'#1E2761' }}>Sources:</strong> SB1 89th Legislature GAA; HB500 (enacted June 22, 2025);
            Agency LARs FY2026-27; LBB CC Issue Dockets; DIR Strategic Plan. Est. figures are directional.
            Tags marked ↗ have sourced context — click for details.
          </p>
        </div>
      )}
        ...
        <div style={{ maxWidth:1400, margin:'0 auto', padding:'24px' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(220px,1fr))', gap:12, marginBottom:24 }}>
            {[
              ...
              <div key={c.label} style={{ background:c.color, borderRadius:12, padding:'16px 18px', border:`1px solid ${c.color}` }}>
          ...
          <div style={{ display:'flex', gap:8, marginBottom:16, flexWrap:'wrap' }}>
            {[
              ...
              <button key={f.key} onClick={() => setLegFilter(f.key)} style={{
                ...
                fontWeight:  legFilter===f.key ? 500 : 400,
              }}>{f.label}</button>
            ))}
          </div>
          ...
          {legFiltered.map(agency => (
            <div key={agency.id} style={{ background:'#FFFFFF', borderRadius:12, border:'1px solid rgba(30,39,97,0.1)', marginBottom:16, overflow:'hidden', boxShadow:'0 2px 8px rgba(30,39,97,0.05)' }}>
                  ...
                  {(agency.sb1_items||[]).length > 0 && (
                    <div>
                      ...
                      {agency.sb1_items.map((item, i) => (
                        <div key={i} style={{ background:'#F4F8FE', borderRadius:8, padding:'10px 12px', marginBottom:8, borderLeft:'3px solid #378ADD' }}>
                          ...
                          {item.amount && (
                            <div style={{ fontSize:11, fontWeight:600, color:'#1E2761', marginTop:6, fontFamily:'monospace' }}>
                              {item.amount >= 1000 ? `$${(item.amount/1000).toFixed(1)}B` : `$${item.amount}M`} — {item.fy}
                            </div>
                          )}
                        </div>
                      ))}
                  ...
                  {(agency.hb500_items||[]).length > 0 && (
                    <div>
                      ...
                      {agency.hb500_items.map((item, i) => (
                        <div key={i} style={{ background:'#F4FBF0', borderRadius:8, padding:'10px 12px', marginBottom:8, borderLeft:'3px solid #639922' }}>
                          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:4 }}>
                            <span style={{ fontSize:10.5, fontWeight:600, color:'#1E2761' }}>{item.section}</span>
                  ...
                  {(agency.exceptional_items||[]).length > 0 && (
                    <div>
                      ...
                      {agency.exceptional_items.map((item, i) => (
                        <div key={i} style={{ background:'#FFFBF0', borderRadius:8, padding:'10px 12px', marginBottom:8, borderLeft:'3px solid #C8A951' }}>
                          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:4 }}>
      ...
      {/* ── Tag Drawer — slides up on tag click ───────────────────────────── */}
      <TagDrawer
        tag={activeTag?.tag}
        agency={activeTag?.agency}
        onClose={() => setActiveTag(null)}
      />

      {/* ── Legacy AgencyModal — kept as fallback ─────────────────────────── */}
      {modal && (
